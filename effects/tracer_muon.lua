...
function EFFECT:Render()
	...
	if ShouldDrawGlobalParticles(self.EndPos) then
	...
	for i = 0, nlen, 32 do
		...
		render.DrawBeam(prevspinpos, spinbeampos, 10, texcoord + i, texcoord + nlen / 128 + i, Color(175, 36, 192, 255 * alpha))

		prevspinpos = spinbeampos
	end

	for i = 0, 4 do
		render.DrawBeam(self.StartPos, self.EndPos, 6, texcoord, texcoord + nlen / 128, Color(95, 95, 105, 255 * alpha))
	end
	render.DrawBeam(self.StartPos, self.EndPos, 18, texcoord, texcoord + nlen / 128, Color(185, 23, 215, 250 * alpha))

	render.SetMaterial(glowmat)
	render.DrawSprite(self.StartPos, 50, 50, Color(145, 120, 172, 178 * alpha))
	render.DrawSprite(self.EndPos, 50, 50, Color(145, 120, 172, 178 * alpha))
end
