...
function ENT:OnInitialize()
	rawset(PLAYER_status_drown, self:GetOwner(), self)
end

function ENT:OnRemove()
	rawset(PLAYER_status_drown, self:GetOwner(), nil)
end
...
function ENT:GetDrownTime()
	...
	if owner:IsValid() and owner:HasTrinket("trinket_oxygentank") then
		return 300
	end
