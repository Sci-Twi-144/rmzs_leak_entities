...
function ENT:Processing() --yeah fuck around with it
	timer.Simple(0, function()
		local owner = self:GetOwner()
		local stacks = self:GetStacks()
		local mystacks = owner.RenegadeStacks
		local adds = owner:GetStatus("renegadeb") and owner:GetStatus("renegadeb"):GetStacks() or 0
		owner.RenegadeStacks = math.min(stacks + mystacks, (1 + adds) or 1)
	end)
end
