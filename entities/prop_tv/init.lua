...
function ENT:Use(activator, caller)
	...
	if activator.Camera and activator.Camera:IsValid() then
		...
		hook.Add("SetupPlayerVisibility", tostring(self), function(pl)
			if not IsValid(self) then return end

			if not (activator.Camera and activator.Camera:IsValid()) then return end

			if (tv.Viewers and tv.Viewers[activator]) then
