...
function ENT:AttachTo(baseent, attachent, physbone, physbone2)
	...
	if baseent:GetBarricadeHealth() == 0 then
		...
		--Should work if that prop is nailed first time .. right?

		local model = baseent:GetModel():lower() -- Only count mdls

		if string.find( model, "mdl" ) then
			GAMEMODE.StatTracking:IncreaseElementKV(STATTRACK_TYPE_PROPS, model, "count", 1)
		end
	...
	parent:SetDTInt(DT_AMOUNT_NAILS, baseent:NumLivingNails())
end
...
function ENT:SetAttachEntity(ent, physbone1, physbone2)
	...
	timer.Simple(0, function() GAMEMODE:EvaluatePropFreeze(ent) end)
...
function ENT:SetDeployer(pl)
	...
	elseif pl:IsValid() then
		...
		if pl.HpPerNailBounty then
			self.BonusHp = pl.HpPerNailBounty
		end
	end
end
...
function ENT:OnRemove()
	...
	if parent:IsValid() and baseent:IsValid() then
		parent:SetDTInt(DT_AMOUNT_NAILS, baseent:NumLivingNails())
	end
