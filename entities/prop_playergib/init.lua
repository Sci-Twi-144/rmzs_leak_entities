...
function ENT:StartTouch(ent)
	if self.DieTime ~= 0 and ent:IsValidLivingZombie() and ent:Health() < ent:GetMaxZombieHealth() and not ent:GetStatus("shockdebuff") then
		self.DieTime = 0

		local capped = ent:GetMaxZombieHealth() * ((ent:GetBossTier() >= 2 and 0.15) or ((ent:GetBossTier() >= 1) and 0.3) or 1)
		if ent:Health() < capped then
			ent:SetHealth(math.min(capped, ent:Health() + 15))
		end
