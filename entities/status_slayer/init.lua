...
function ENT:CreateSVHook(enty)
	hook.Add("SpecialPlayerDamage", tostring(enty), function(ent, dmginfo)
		...
		if attacker:IsValid() and attacker:IsPlayer() and attacker:Team() == TEAM_HUMAN and dmginfo:GetInflictor().IsMelee then
			dmginfo:SetDamage(dmginfo:GetDamage() * (1 + 0.01 * enty:GetStacks()))
		end
	end)
