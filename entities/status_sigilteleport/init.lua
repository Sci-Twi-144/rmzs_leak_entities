...
function ENT:PlayerSet(pPlayer, bExists)
	...
	hook.Add("SpecialPlayerDamage", ENTC, function(ent, dmginfo)
		...
		if ent == owner and (attacker:IsValid() and (attacker == owner or (owner:IsPlayer() and attacker:IsPlayer() and attacker:Team() ~= owner:Team()))) and not dmginfo:GetInflictor().IsStatus then
			...
			enty:SetEndTime(CurTime() +2 * (ent.SigilTeleportTimeMul or 1))
		end
	end)
...
function ENT:Think()
	...
	if (CurTime() >= self:GetEndTime() - 0.5) and self:GetClearPlace() and self.Push then
		if target and target:IsValid() then
			if owner:IsValidLivingHuman() then
				for _, ent in pairs(ents.FindInSphere(target:GetPos() + Vector(0, 0, 6), 32)) do
					if ent:IsValid() and ent:IsValidLivingZombie() then
						ent:ThrowFromPositionSetZ(target:GetPos() + Vector(math.random(-1,1), math.random(-1,1), 0), 200, nil, true)
					end
				end
			end
		end
	...
	if CurTime() >= self:GetEndTime() then
		owner:DoSigilTeleport(target, froms, self:GetClass() == "status_corruptedteleport")
		self:Remove()
	end
