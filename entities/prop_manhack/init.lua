...
function ENT:Initialize()
	...
	--[[ слишком жрет фпс и бесполезно в текущем зсе
...
function ENT:OnTakeDamage(dmginfo)
	...
	if dmginfo:GetDamageType() == DMG_IMPACT then
		local modifier = self:GetImpactDamageModifier()
...
function ENT:PhysicsSimulate(phys, frametime)
	...
	local autocontrol = owner:IsSkillActive(SKILL_AUTOCONTROL)

	local trace = {mask = MASK_HOVER, filter = self, start = self:GetPos()}
	local tr, tr2

	if self:BeingControlled() or autocontrol then
		trace.endpos = trace.start - Vector(0, 0, self.HoverHeight * 0.333)
		tr = util.TraceLine(trace)

		if not autocontrol then
			...
			if not IsValid(self.Target) and (self.SeekTimer < CurTime()) then
				self.SeekTimer = CurTime() + 2 -- бля пиздец сервер оно нагружает
				local radius = 1024
				...
				for _, ent in pairs(util.BlastAlloc(self, owner, pos, radius)) do
					if ent:IsValidLivingZombie() then
						...
						if WorldVisible(pos, ent:GetPos()) and not(classtab.IsHeadcrab or classtab.IsTorso or SpawnProtection[ent]) then
							self.Target = ent
							break
						end
					end
			...
			if IsValid(self.Target) and (SpawnProtection[self.Target] or not WorldVisible(self:GetPos(), self.Target:GetPos())) then 
				self.Target = nil 
			end
	
			if IsValid(self.Target) then
				local target = self.Target
				targetpos = target:LocalToWorld(target:OBBCenter() + Vector(0, 0, 1))
				direction = (targetpos - self:GetPos()):GetNormal()
				movedir = direction
				self:SetAngles(direction:Angle()) -- возможно стоит убрать
			end
		end
