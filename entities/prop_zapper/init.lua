...
function ENT:FindZapperTarget(pos, owner)
	...
	for k, ent in pairs(util.BlastAlloc(self, owner, pos, self.Range * (owner.FieldRangeMul or 1))) do
...
function ENT:Think()
	...
	if curammo >= 2 and owner:IsValid() then
		...
		if target then
			...
			local damage = self.Damage * (0.1 * 4) -- too lazy to fix it right now
			target:AddLegDamageExt(damage * 0.33, owner, self, SLOWTYPE_PULSE)
			timer.Simple(0, function()
				target:AddLegDamage(damage * 0.67)
			end)

			--if self.PointsMultiplier then
			--	POINTSMULTIPLIER = self.PointsMultiplier
			--end
			target:TakeSpecialDamage(target:IsHeadcrab() and self.HeadcrabDamage or self.Damage, DMG_SHOCK, owner, self)
			--if self.PointsMultiplier then
