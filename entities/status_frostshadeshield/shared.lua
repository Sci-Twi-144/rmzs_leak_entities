...
function ENT:SetObjectHealth(health)
	...
	if health <= 0 and not self.Destroyed then
		...
		if SERVER then
			...
			for _, ent in pairs(util.BlastAlloc(self, owner, owner:GetPos(), 128)) do
				if ent:IsValidLivingPlayer() and gamemode.Call("PlayerShouldTakeDamage", ent, owner) and ent ~= owner then
					--ent:GiveStatus("frost", 7)
					--ent:ApplyZombieDebuff("frost", 7, {Applier = owner}, true, 10)
					ent:AddLegDamageExt(18, owner, self, SLOWTYPE_COLD)
				end
			end

			owner:GodEnable()
			util.BlastDamageEx(self, owner, owner:GetPos(), 128, 30, DMG_GENERIC)
			owner:GodDisable()
