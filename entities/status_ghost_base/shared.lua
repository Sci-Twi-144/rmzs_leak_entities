...
function ENT:RecalculateValidity()
	...
	local function CheckOwnerDistance(entic, posit)
		...
		if trash[self.GhostEntity] then
			return true
		end
		
		local objowner = entic:GetObjectOwner() or false
		
		if not objowner then
			return true
		else
			local alloweddistance = (owner != objowner) and math.floor(self.GhostDistance * 0.025) or 10
			if owner != objowner then
				return (alloweddistance * alloweddistance) >= posit:DistToSqr(entic:NearestPoint(posit))
			else
				return true
			end
		end
	...
	if tr.HitWorld and not tr.HitSky or tr.HitNonWorld and self.GhostPlaceOnEntities then
		...
		elseif self.GhostDistance then
			for _, ent in pairs(ents.FindInSphere(tr.HitPos, self.GhostDistance)) do
				if ent and ent:IsValid() and WorldVisible(tr.HitPos, ent:NearestPoint(tr.HitPos)) then
					if self.GhostEntityWildCard then
						if self.GhostEntityWildCard == ent:GetClass():sub(1, #self.GhostEntityWildCard) and CheckOwnerDistance(ent, tr.HitPos) then
							valid = false
							break
						end
					elseif ent:GetClass() == self.GhostEntity and CheckOwnerDistance(ent, tr.HitPos) then
						valid = false
						break
					end
				end
