...
function ENT:Think()
	...
	if owner:IsValid() and owner:IsPlayer() and owner:Alive() and owner:Team() == TEAM_UNDEAD and owner:GetZombieClassTable().Name == "Cum-Shade" then
		local ent = self:GetParent()
		if ent:IsValid() then
			local eyepos = owner:EyePos()
			if eyepos:DistToSqr(ent:NearestPoint(eyepos)) <= 160000 then --400^2
				local phys = ent:GetPhysicsObject()
				if phys:IsValid() and phys:IsMoveable() and phys:GetMass() <= 7500 then
					local ct = CurTime()
					...
					ShadowParams.angle = self:GetParent():GetClass() == "projectile_shadecum" and eyeangles or AngleRand()
					ShadowParams.maxangular = 100
					ShadowParams.maxangulardamp = 200
