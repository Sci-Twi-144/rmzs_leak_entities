...
function ENT:AcceptInput(name, activator, caller, args)
	...
	elseif name == "spawndemiboss" then
		GAMEMODE:SpawnMinibossZombieFGD(false, self.Silent, self.BossIndex, true)
	...
	elseif name == "disable" then
		self.On = false
		return true
	elseif name == "setsilent" or name == "setinstantchange" or name == "setclass" or name == "setdemiclass" then
...
function ENT:KeyValue(key, value)
	...
	elseif key == "class" or key == "demiclass" then
		local demiclass = key == "demiclass"
		self.DemiBoss = demiclass
		self.BossIndex = nil
		...
		for _, classtable in pairs(GAMEMODE.ZombieClasses) do
			if string.lower(classtable.Name) == (demiclass and "onyx nightmare" or "nightmare") then
				self.BossIndex  = classtable.Index
				break
			end
		end
...
function ENT:StartTouch(ent)
	if self.On and ent:IsPlayer() and ent:Alive() and ent:Team() == TEAM_UNDEAD then
		if ent:GetZombieClassTable().Boss then
			...
			if self.DemiBoss then
			GAMEMODE:SpawnMinibossZombieFGD(ent, self.Silent, self.BossIndex, true)
			else
