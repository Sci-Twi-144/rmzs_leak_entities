...
function ENT:AttachTo(ent)
	...
	self:SetModelScale((ent:GetModelScale() or 1)+0.03, 0)
		...
		if ent:IsValid() then
			...
			if eyepos:DistToSqr(ent:NearestPoint(eyepos)) <= 160000 then --400^2
				...
				if phys:IsValid() and phys:IsMoveable() and phys:GetMass() <= 300 then
					...
					ShadowParams.maxangular = 100 //What should be the maximal angular force applied
					ShadowParams.maxangulardamp = 200 // At which force/speed should it start damping the rotation
