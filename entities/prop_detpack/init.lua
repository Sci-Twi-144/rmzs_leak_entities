...
function ENT:Initialize()
	...
	self:SetARMTime(self:GetOwner().InstantDetonation and 2.5 or 5)
...
function ENT:Explode()
	...
	if owner:IsValidHuman() then
		...
		util.BlastDamagePlayer(self, owner, pos, 256, 480 * mul, DMG_ALWAYSGIB)
...
function ENT:SetExplodeTime(time)
	if self.CreateTime + self:GetARMTime() > CurTime() then return end
