...
function ENT:DrawTranslucent()
	...
	if not camera and (P_Team(MySelf)~=TEAM_HUMAN or MySelf:KeyDown(IN_SPEED)) then
	...
	if lp:IsValid() and lp:Team() == TEAM_HUMAN and owner:IsValid() and owner:IsPlayer() and not ShouldVisibleDraw(self:GetPos()) then
		...
		cam.Start3D2D(camera and adjvec or self:LocalToWorld(normalvec), ang, 0.03)
			...
			if classname == "prop_drone" then
				draw.SimpleText(self:CanShoot() and "Ready to shoot" or "Too far to the nearest target", "ZS3D2DFontSmall", 0, 230, self:CanShoot() and COLOR_GREEN or COLOR_RED, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			end
