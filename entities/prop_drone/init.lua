...
function ENT:CreateHook()
	...
	hook.Add("SetupPlayerVisibility", ENTC, function(pl)
		...
		if pl ~= enty:GetObjectOwner() then return end

		AddOriginToPVS(enty:GetPos())
		AddOriginToPVS(enty:GetPos() + pl:GetAimVector() * 1024)
	end)
end
...
function ENT:RemoveHook()
	...
	hook.Remove("SetupPlayerVisibility", ENTC)
end
...
function ENT:Use(activator, caller)
	if not activator:IsPlayer() or activator:Team() ~= TEAM_HUMAN or not self:GetObjectOwner():IsValid() or (activator:GetInfo("zs_nousetodeposit") ~= "0" and self:GetObjectOwner() ~= activator) then return end
...
function ENT:FireTurret(src, dir)
	...
	if self:GetNextFire() <= CurTime() then
		...
		if curammo > 0 then
			...
			self:FireBulletsLua(src, dir, 5, 1, 1, 1, 19, owner, nil, "AR2Tracer", self.BulletCallback, nil, nil, self.GunRange, nil, self)
...
function ENT:Think()
	...
	if self:GetAmmo() > 0 then
		if self:BeingControlled() and owner:KeyDown(IN_ATTACK) and ( self:CanShoot() ) then
