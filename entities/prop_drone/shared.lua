...
function ENT:BeingControlled()
	...
	if owner:IsValid() then
		...
		return wep:IsValid() and wep:GetClass() == "weapon_zs_dronecontrol" and wep:GetDTBool(7)
	end
...
local function ManualTraceFilter(ent)
	if ent == temp_attacker or getmetatable(ent) == M_Player and P_Team(ent) == TEAM_HUMAN or ent.FHB or ent.ScanFilter or ent.IsCreeperNest then
		return false
	end
...
function ENT:GetImpactDamageModifier()
	local wave = GAMEMODE:GetWave()
	return (wave <= 2 and 1) or (0.25 * wave + 0.5)
end
...
function ENT:GetTrace()
	...
	return util.TraceLine({start = epos, endpos = epos + ang:Forward() * self.GunRange * (owner.DroneGunRangeMul or 1), mask = MASK_SHOT, 
		filter = self:GetTraceFilter()
	})
end

function ENT:CanShoot()
	local normal = self:GetTrace().HitNormal

	return not (self:GetClass() == "prop_drone" and normal == vector_origin)
end