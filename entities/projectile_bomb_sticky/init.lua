...
function ENT:Explode()
	...
	if owner:IsValidLivingHuman() then
		...
		local pos = self:GetPos()
		local radius = self.ProjRadius or 70
		local taper = 1
		if alt then
			local source = self:ProjectileDamageSource()
			for _, ent in pairs(util.BlastAlloc(source, owner, pos, radius * (owner.ExpDamageRadiusMul or 1))) do
				if ent:IsValidLivingPlayer() and gamemode.Call("PlayerShouldTakeDamage", ent, owner) then
					ent:AddLegDamageExt(((self.ProjDamage * 1.2) or 75) * self:GetCharge() * taper, owner, source, SLOWTYPE_COLD)
					taper = taper * 0.85
				end
			end
		...
		if not alt then
			...
			for _, ent in pairs(util.BlastAlloc(self, owner, pos, (self.ProjRadius * (owner.ExpDamageRadiusMul or 1)) * (0.4 * taper))) do
				if ent:IsValidLivingZombie() and gamemode.Call("PlayerShouldTakeDamage", ent, owner) then
		...
		util.BlastDamagePlayer(source, owner, pos, radius, (self.ProjDamage or 75) * self:GetCharge(), DMG_ALWAYSGIB, self.ProjTaper or 0.6)
		if alt then
			POINTSMULTIPLIER = 1
		end
	end
