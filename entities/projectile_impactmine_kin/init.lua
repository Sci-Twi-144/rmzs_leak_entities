...
function ENT:Hit(vHitPos, vHitNormal, eHitEntity, vOldVelocity)
	...
	--self:SetCustomGroupAndFlags(ZS_COLLISIONGROUP_DEPLOYABLE, ZS_COLLISIONGROUP_ALL)
...
local function BulletCallback(attacker, tr, dmginfo)
	...
	if ent:IsValid() then
		...
		if ent:IsPlayer() and math.random(3) == 1 then
			ent:AddBurnDamage(30, attacker, attacker.BurnTickRate or 1, true)
		end
	end
end
...
function ENT:Explode()
	...
	self:FireBulletsLua(pos, self:GetUp(), 3, 3, 1, 1, self.ProjDamage or 26.67, self:GetOwner(), 0.01, "AR2Tracer", BulletCallback, nil, nil, nil, nil, self)
