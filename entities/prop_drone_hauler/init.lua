...
ENT.GrappledEnt = nil
local carryitemsclasses = {"prop_ammo", "prop_weapon", "prop_invitem"}
local carrypropclasses = {"prop_physics", "prop_physics_multiplayer", "func_physbox"}
function ENT:RopeAttach()
	...
	if tr.Hit and ropetraceent and ropetraceent:IsValid() then
		...
		if ( table.HasValue(carryitemsclasses, entclass) or ( not owner.NoObjectPickup and table.HasValue(carrypropclasses, entclass) ) ) then
