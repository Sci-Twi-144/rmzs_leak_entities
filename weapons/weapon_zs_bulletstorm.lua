...
if CLIENT then
	...
	function SWEP:Draw2DHUDAds(x, y, hei, wid)
		local maxbullets, bulletsleft = self:GetPrimaryClipSize(), self:GetBullets()
...
function SWEP.BulletCallback(attacker, tr, dmginfo)
	...
	if SERVER and ent:IsValidLivingZombie() and wep:GetHitStacks() >= 2 then
		wep:SetHitStacks(0)
		wep:SetBullets(math.min(wep:GetBullets() + 1, wep:GetPrimaryClipSize()))
	end
...
function SWEP:PrimaryAttack()
	...
	local stbl = E_GetTable(self)
		if self:GetFireMode() == 0 and self:Clip1() > 0 then -- Первый режим тратит обычные патроны, проверка на клип нужна в избежании бага с Q режимом
			self:TakeAmmo()
...
function SWEP:CanPrimaryAttack()
	...
	if (self:Clip1() < stbl.RequiredClip) and self:CanReload() and self:GetFireMode() == 0 then
		self:Reload()
		return false
	end

	if self:Clip1() < stbl.RequiredClip and self:GetFireMode() == 0 then
...
-- function SWEP:OnZombieKilled(zombie, total, dmginfo)
	...
	-- if killer:IsValidLivingHuman() and self:GetFireMode() == 0 then
		-- self:SetBullets(math.min(self:GetBullets() + 2, self:Clip1()))
	-- end
-- end
