...
SWEP.Description = (translate.Get("desc_ripper"))
...
function SWEP:TakeAmmo(secondary)
	self:TakePrimaryAmmo(secondary and 2 or 1)
end

function SWEP:SecondaryAttack()
	if self:Clip1() < 2 or not self:CanPrimaryAttack() then return end
	...
	if SERVER then
	self:ShootSecondary(self.Primary.Damage, self.Primary.NumShots, self:GetCone())
	end
	self.IdleAnimation = CurTime() + self:SequenceDuration()
end
