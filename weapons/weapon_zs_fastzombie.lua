...
function SWEP:Think()
	...
	if self:GetClimbing() and not GAMEMODE.ZombieEscape then
		...
		if curtime >= self:GetSlowSwingEnd() then
			...
			if IsFirstTimePredicted() then
				...
				for i=1, #traces do
					...
					if not (trace or trace.Hit) then continue end
...
function SWEP:SecondaryAttack()
	if self:IsPouncing() or self:IsClimbing() or self:GetPounceTime() > 0 or GAMEMODE.ZombieEscape then return end
	...
	elseif self:GetClimbSurface() and not GAMEMODE.ZombieEscape then
		self:StartClimbing()
	end
end
...
function SWEP:Reload()
	...
	if CurTime() < self:GetNextSecondaryFire() or self:GetClimbing() or self:IsPouncing() or self:GetPounceTime() > 0  then return end
