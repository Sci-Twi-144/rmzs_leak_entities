...
function SWEP:BuildingThink()
	...
	for _, sigil in pairs(ents.FindByClass("prop_obj_sigil")) do
		...
		if util.SkewedDistance(sigil:GetPos(), hitpos, 1.5 * (GAMEMODE.CreeperNestVerticalSkewMultiplier or 1)) <= (GAMEMODE.CreeperNestDistBuildNest * (GAMEMODE.CreeperNestDistBuildMultiplier or 1)) then
	...
	for _, human in pairs(team.GetPlayers(TEAM_HUMAN)) do
		if util.SkewedDistance(human:GetPos(), hitpos, 1.5 * (GAMEMODE.CreeperNestVerticalSkewMultiplier or 1)) <= (GAMEMODE.CreeperNestDistBuild * (GAMEMODE.CreeperNestDistBuildMultiplier or 1)) then
