...
function SWEP:OnMeleeHit(hitent, hitflesh, tr)
	...
	if hitent.GetObjectHealth then
		...
		local healstrength = self.Repair * (owner.RepairRateMul or 1) * (hitent.WrenchRepairMultiplier or 1)

		hitent:SetObjectHealth(math.min(hitent:GetMaxObjectHealth(), hitent:GetObjectHealth() + self.Repair))
