...
function SWEP:Think()
	...
	if self:IsPouncing() then
		...
		if owner:WaterLevel() >= 2 then
			...
			local ACD = self:GetNextPrimaryFire() - CurTime()
...
function SWEP:SecondaryAttack()
	local owner = self:GetOwner()
	if CurTime() < (self:GetNextSecondaryFire() + self:GetAttackProcessTime()) then return end
	...
	self:SetNextSecondaryFire(CurTime() + self.Secondary.Delay + self.AttackProcessTime)

	local SCD = (self.Secondary.Delay + self:GetAttackProcessTime()) - CurTime()
...
function SWEP:SetCooldown(time)
	self:SetDTFloat(9, CurTime() + time)
end
...
function SWEP:SetTime(time)
	self:SetDTFloat(11, CurTime() + time)
end
