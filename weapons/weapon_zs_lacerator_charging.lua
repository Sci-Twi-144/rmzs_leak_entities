		...
		elseif IsFirstTimePredicted() then
			...
			for _, trace in ipairs(traces) do
				...
				if trace.HitWorld then
					...
					if ent and ent:IsValid() and not (ent:IsProjectile() or ent.ZombieConstruction or ent.IsShadeShield) then
						hit = true
						...
						if ent:IsPlayer() then
							...
							if CurTime() >= (ent.NextKnockdown or 0) and self:IsChargeCritical() then
								if SERVER then
									ent:ApplyZombieDebuff("knockdown", self.ChargeKnockdown * chargemul, {Applier = owner}, true, 13)
								end
