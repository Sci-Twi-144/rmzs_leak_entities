...
function SWEP:DoAlert()
	...
	for _, ent in pairs(ents.FindInSphere(screampos, 92)) do
		if ent and ent:IsValidHuman() then
			...
			local blocklimit = (GAMEMODE.BlockLimit * (ent.BlockLimitMul or 1)) * (ent:GetActiveWeapon().BlockStability or 1)
			local dmgclamped = math.min(math.max(0, (blocklimit * 0.1)), blocklimit)
			if SERVER then
				ent:SetDamageBlocked(ent:GetDamageBlocked() + dmgclamped)
			end
		--	ent.GuardBreak = true
			...
			if dist <= 92 and TrueVisible(entearpos, screampos) then
				...
				local ppower = power * 0.15
				viewpunch(ent, power)
				for i=1, 5 do
					if SERVER then
						ent:SetDamageBlocked(ent:GetDamageBlocked() + (dmgclamped * 0.5))
					end
