...
SWEP.PrintName = (translate.Get("wep_slugrifle"))
SWEP.Description = (translate.Get("desc_slugrifle"))
SWEP.Slot = 3
SWEP.SlotPos = 0
...
GAMEMODE:AttachWeaponModifier(SWEP, WEAPON_MODIFIER_RELOAD_SPEED, 0.137)
GAMEMODE:AttachWeaponModifier(SWEP, WEAPON_MODIFIER_FIRE_DELAY, -0.1, 1)

GAMEMODE:AddNewRemantleBranch(SWEP, 1, (translate.Get("wep_slugrifle_exp")), (translate.Get("desc_slugrifle")), function(wept)
		wept.Primary.ClipSize = 8
		wept.RequiredClip = 2
		wept.ReloadSpeed = 0.9
		wept.HeadshotMulti = 2.35
		wept.Pierces = 3
		wept.DamageTaper = 0.45
		wept.IsAoe = true
		...
		wept.OnZombieKilled = function(self, zombie, total, dmginfo)
			...
			if killer:IsValidLivingHuman() and minushp > 10 and zombie:WasHitInHead() then -- 10
				local pos = zombie:GetPos()
				if dmginfo:GetInflictor() == killer:GetActiveWeapon() then
					timer.Simple(0.06, function()
						if killer:IsValidLivingHuman() then
							util.BlastDamagePlayer(killer:GetActiveWeapon(), killer, pos, 104 * (killer.ExpDamageRadiusMul or 1), minushp, DMG_ALWAYSGIB, 0.85)
						end
					end)
				end
...
local br = GAMEMODE:AddNewRemantleBranch(SWEP, 2, (translate.Get("wep_berdan")), (translate.Get("desc_berdan")), "weapon_zs_berdan")
br.Colors = {Color(190, 90, 60), Color(160, 60, 30), Color(130, 30, 00)}
...
function SWEP.BulletCallback(attacker, tr, dmginfo)
	if tr.HitGroup == HITGROUP_HEAD then
		local ent = tr.Entity
		if ent:IsValidLivingZombie() and (ent:GetBossTier() > 1) then
			return
		end

		if gamemode.Call("PlayerShouldTakeDamage", ent, attacker) then
			dmginfo:SetDamageType(DMG_DIRECT)
			local percentage = ent:IsValidLivingZombie() and 1/(ent:GetBossTier() * 5 + 1) or 0.1
			dmginfo:SetDamage(dmginfo:GetDamage() + ent:GetMaxHealthEx() * percentage)
		end
	end
end
