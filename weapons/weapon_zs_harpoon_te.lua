...
SWEP.PrintName = (translate.Get("wep_harpoon_tet"))
SWEP.Description = (translate.Get("desc_harpoon_tet"))
...
function SWEP:SecondaryAttack()
	...
	if SERVER then
		local ent = ents.Create("projectile_harpoon_te1")
		if ent:IsValid() then
			ent:SetPos(owner:GetShootPos())
			ent:SetAngles(owner:EyeAngles())
			ent:SetOwner(owner)
			ent:SetPuller(owner)
			ent.ProjDamage = self.MeleeDamage * (owner.ProjectileDamageMul or 1) * 0.6
