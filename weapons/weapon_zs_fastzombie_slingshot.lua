...
function SWEP:SecondaryAttack()
	if self:IsPouncing() or self:IsClimbing() or self:GetPounceTime() > 0 and GAMEMODE.ZombieEscape then return end
...
function SWEP:Slingshot()
	...
	if owner:IsOnGround() then
		...
		if SERVER then
			...
			local hpmul = math.min((owner:Health() * 0.6) / owner:GetMaxHealth(), 1)
			local spawnpos = pos
			...
			if classtable then
				...
				timer.Simple(0, function()
					if owner:IsValidLivingZombie() then
						...
						owner:SetHealth(math.max(1, math.min(owner:GetMaxHealth() * hpmul, owner:GetMaxHealth())))
